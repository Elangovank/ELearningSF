...
import com.gm.controllers.adapter.SelectedSupportDetailsAdapter
...
import kotlinx.android.synthetic.main.fragment_selected_support_detail.*
import kotlinx.android.synthetic.main.item_audio_player.view.*
import kotlinx.android.synthetic.main.item_check_list.*
import kotlinx.android.synthetic.main.item_confirmation_dialog.view.*
import kotlinx.android.synthetic.main.item_support_details.*
...
class SelectedSupportDetailFragment : GMBaseFragment(), OnItemClickListener {
    ...
    var listValues = ArrayList<Model.SupportQuestion>()
    ...
    private var offlineSupport = Model.OfflineSupportMediaList()
    ...
    override fun onItemSelected(item: Any?, selectedIndex: Int) {
        ...
        } else if (item is Model.SupportQuestion) {
            val selectedItem = item.response ?: false
            listValues.get(selectedIndex).response = !selectedItem
    ...
    companion object {
        ...
        const val ARG_SELECTED_ITEM_IMAGE_URL = "ARG_SELECTED_ITEM_IMAGE_URL"
        const val ARG_SELECTED_SUB_CATAGORY = "Sub_Catogory"


        fun newInstance(args: Bundle): SelectedSupportDetailFragment {
            val fragment = SelectedSupportDetailFragment()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_selected_support_detail, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setResourceString()
        var title = (arguments?.getString(ARG_SELECTED_ITEM) ?: "")
        var imageUrl = (arguments?.getString(ARG_SELECTED_ITEM_IMAGE_URL) ?: "")
        initToolbar(arguments?.getString(ITEM_TITLE) ?: "")
        toggleVisibility()
        recyclerView?.isNestedScrollingEnabled = false
        titleTextView?.text = title
        mediaTextView.text = getResourceString("capture_problem")
        ImageUtils.loadImageToGlide(titleImageView, imageUrl)
        getSupportQuestionList()

        uploadTextView.setOnClickListener {
            if ((listValues.filter { it.response == true }?.size
                            ?: 0) != 0 || !commentEditText.text.toString().equals(""))
    ...
    private fun toggleClickableAction(status: Boolean) {
        ...
        uploadTextView?.isEnabled = status
        recyclerView?.isEnabled = status
        if (status) {
    ...
    fun disable(status: Boolean, list: ArrayList<Model.SupportQuestion>) {
        list.forEach {
            listTextView.isClickable = status
        }
    }
    ...
    private fun toggleVisibility() {
        ...
        uploadTextView.visibility = View.VISIBLE
    ...
    private fun getSupportQuestionList() {
        showProgressBar()
        ServiceWrapper.getSupportQuestionList(arguments?.getLong(ARG_SELECTED_SUB_CATAGORY)?.toInt()
                ?: 0, 0, object : ServiceRequestListener {
            override fun onRequestCompleted(responseObject: Any?) {
                dismissProgressBar()
                if (responseObject is Model.SupportQuestionListResponse) {
                    responseObject.supportQuestionList?.let {
                        listValues = it
                        listItems()
                    }
                }
    ...
    fun setResourceString()
    {
        ...
        emptyTextView?.text=getResourceString("no_questions_found")
        commentGMTextInputLayout?.hint=getResourceString("comment")
        uploadTextView?.text=getResourceString("submit")
    }
    ...
    private fun initAdapter() {
        ...
        recyclerView?.adapter = SelectedSupportDetailsAdapter(listValues, this)
    }
            ...
            val runnable = object : Runnable {
                override fun run() {
                    ...
                    if (mediaList.size != 0) {
                        ...
                        if ((!commentEditText?.text.toString().equals("")) || (listValues.filter { it.response == true }?.size
                                        ?: 0) != 0)
                            createSupportTicket(true)
                        else showSnackBar(getResourceString("messege_support_submit"))
                    }
    ...
    private fun createSupportTicket(showProgress: Boolean = false) {
       // if (showProgress)
        // showProgressBar()
        var submitList = Model.SupportCreateTicket()
        submitList.userId = GMApplication.loginUserId
        submitList.batchId = 0
        submitList.comment = commentEditText?.text.toString()
        submitList.customerSupportSubCategoryId = arguments?.getLong(ARG_SELECTED_SUB_CATAGORY)
        submitList.questions = ArrayList(listValues.mapNotNull { if (it.response == true) it.supportQuestionId else null })
        submitList.medias = mediaUploaded
        submitList.let {
            DataProvider.createSupportTicket(it, object : ServiceRequestListener {
                override fun onRequestCompleted(responseObject: Any?) {
                    dismissProgressBar()
                    if (responseObject is Model.SupportCreateTicketResponse) {
                        context?.let {
                            ...
                            (activity as HomeActivity).replaceFragment(SupportFragment.newInstance(Bundle()), "")
                        }
                    } else {
    ...
    private fun uploadMedia(position: Int) {
        if (position < mediaList.size) {
            ...
            mediaItem.let {
                ...
                offlineSupport.mediaType = it.MediaTypeId
                offlineSupport.mediaPath = it.file?.absolutePath
                DataProvider.uploadSupportMedia(offlineSupport, object : ServiceRequestListener {
                    override fun onRequestCompleted(responseObject: Any?) {
